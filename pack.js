...
for (const [lat, lon] of [
  ...
  [44.928, -87.1853], // fix #13
  [50.7029, -57.3511], // fix #13
  [29.9414, -85.4064], // fix #14
  [49.7261, -1.9104], // fix #15
  [65.528, 23.557], // fix #16
  [35.8722, -84.525], // fix #18
  [60.0961, 18.797], // fix #23
  [59.9942, 18.7794], // fix #23
  [59.05, 15.0412], // fix #23
  [60.027, 18.7594], // fix #23
...
function clip(polygon, min_lat, min_lon, max_lat, max_lon) {
  ...
  for (let i = 0; i < p.length; i++) {
    ...
    if (a[0] >= min_lon !== b[0] >= min_lon) {
      q.push([
        min_lon,
        a[1] + ((b[1] - a[1]) * (min_lon - a[0])) / (b[0] - a[0]),
      ]);
    }
    if (b[0] >= min_lon) {
      q.push(b);
  ...
  for (let i = 0; i < q.length; i++) {
    ...
    if (a[1] >= min_lat !== b[1] >= min_lat) {
      p.push([
        a[0] + ((b[0] - a[0]) * (min_lat - a[1])) / (b[1] - a[1]),
        min_lat,
      ]);
    }
    if (b[1] >= min_lat) {
  ...
  for (let i = 0; i < p.length; i++) {
    ...
    if (a[0] <= max_lon !== b[0] <= max_lon) {
      q.push([
        max_lon,
        a[1] + ((b[1] - a[1]) * (max_lon - a[0])) / (b[0] - a[0]),
      ]);
    }
    if (b[0] <= max_lon) {
      q.push(b);
  ...
  for (let i = 0; i < q.length; i++) {
    ...
    if (a[1] <= max_lat !== b[1] <= max_lat) {
      p.push([
        a[0] + ((b[0] - a[0]) * (max_lat - a[1])) / (b[1] - a[1]),
        max_lat,
      ]);
    }
    if (b[1] <= max_lat) {
